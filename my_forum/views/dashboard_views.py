...
def get_dashboard():
    def make_resp_body():
        ...
        body["data"]["dashboard_num"] = len(boards)
        body["data"]["dashboard_list"] = make_dashboard(boards)
        return body
    ...
    def make_dashboard(boards):
        ...
        for board in boards:
            dashboard = dict()
            dashboard["board_id"] = board.id
            dashboard["board_name"] = board.name
            posts = sorted(board.posts.all(), key=lambda post: post.create_date, reverse=True)[:g_dashboard_size]
            dashboard["posts"] = [{"post_id": post.id, "post_title": post.title} for post in posts]
